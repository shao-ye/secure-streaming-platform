...
WORKER_API_URL=https://yoyoapi.your-domain.com  # 🔥 修复Bug11: 统一为WORKER_API_URL(无S)
VPS_API_KEY=<VPS_API_KEY>  # 与Workers保持一致
...
export async function handleGetChannelConfig(request, env, channelId) {
  try {
    ...
    const defaultRtmpUrls = {
      ...
      'stream_kcwxuedx': '<RTMP_URL>',
      'stream_kil0lecb': '<RTMP_URL>',
      'stream_noyoostd': '<RTMP_URL>',
      'stream_3blyhqh3': '<RTMP_URL>',
      'stream_8zf48z6g': '<RTMP_URL>',
      'stream_cpa2czoo': '<RTMP_URL>'
    };
...
Invoke-RestMethod -Uri "https://yoyoapi.your-domain.com/api/recording/config/stream_xxx" `
...
# 部署到VPS
ssh root@<VPS_IP> "cd /tmp/github/secure-streaming-platform/vps-transcoder-api && ./vps-simple-deploy.sh"
```
...
```bash
# 测试启动录制
curl -X POST https://yoyo-vps.your-domain.com/api/simple-stream/start-watching \
...
# 检查文件生成
ssh root@<VPS_IP> "ls -la /var/recordings/stream_xxx/"
```
...
class SegmentedRecordingManager {
  constructor() {
    ...
    this.workerApiUrl = process.env.WORKER_API_URL || 'https://yoyoapi.your-domain.com';
...
# 重启服务观察修复日志
ssh root@<VPS_IP> "pm2 restart vps-transcoder-api && pm2 logs --lines 50"
```
...
# 2. 同步到VPS Git目录
ssh root@<VPS_IP> "cd /tmp/github/secure-streaming-platform && git pull"

# 3. 执行部署脚本（会自动安装依赖）
ssh root@<VPS_IP> "/tmp/github/secure-streaming-platform/vps-transcoder-api/vps-simple-deploy.sh"

# 4. 验证依赖安装
ssh root@<VPS_IP> "cd /opt/yoyo-transcoder && npm list node-cron"
```
...
# 3. 强制终止FFmpeg进程模拟崩溃
ssh root@<VPS_IP> "pkill -9 ffmpeg"
...
# 6. 验证文件修复结果
ssh root@<VPS_IP> "ls -la /var/recordings/test_channel/"
```
...
# 5. 检查录制文件状态
ssh root@<VPS_IP> "ls -la /var/recordings/test_network/"
...
# 重新部署VPS
ssh root@<VPS_IP> "cd /tmp/github && ./vps-simple-deploy.sh"
```
