...
```powershell
# Windows PowerShell
Resolve-DnsName tunnel-api.yoyo-vps.5202021.xyz
Resolve-DnsName tunnel-hls.yoyo-vps.5202021.xyz
Resolve-DnsName tunnel-health.yoyo-vps.5202021.xyz
```
...
```powershell
# 测试HTTPS连接
curl -I https://tunnel-api.yoyo-vps.5202021.xyz/health
```
...
# 测试访问
curl https://tunnel-api.yoyo-vps.5202021.xyz/health
```
...
```powershell
# 测试1: DNS解析
$dns1 = Resolve-DnsName tunnel-api.yoyo-vps.5202021.xyz
$dns2 = Resolve-DnsName tunnel-hls.yoyo-vps.5202021.xyz
$dns3 = Resolve-DnsName tunnel-health.yoyo-vps.5202021.xyz
...
# 测试2: HTTPS连接
try {
    $r1 = Invoke-RestMethod -Uri "https://tunnel-api.yoyo-vps.5202021.xyz/health" -TimeoutSec 5
...
try {
    $r2 = Invoke-RestMethod -Uri "https://tunnel-health.yoyo-vps.5202021.xyz" -TimeoutSec 5
...
# 测试3: HLS文件访问（需要有活跃的流）
# 先启动一个测试流，然后访问
# https://tunnel-hls.yoyo-vps.5202021.xyz/hls/stream_xxx/playlist.m3u8
```
...
```powershell
# 测试API请求
$r = Invoke-WebRequest -Uri "https://yoyoapi.5202021.xyz/api/streams" -Headers @{"Authorization"="Bearer YOUR_TOKEN"}
...
```powershell
# 测试tunnel+proxy
$r = Invoke-WebRequest -Uri "https://yoyoapi.5202021.xyz/api/simple-stream/start-watching" `
...
- [ ] tunnel 返回 `tunnel-hls.yoyo-vps.5202021.xyz`
- [ ] direct 返回 `yoyoapi.5202021.xyz`
...
**发现的问题**：
- 隧道模式开启后，浏览器直接访问 `tunnel-hls.yoyo-vps.5202021.xyz` 
