...
```
维度1: Workers → VPS
  - tunnel: tunnel-*.yoyo-vps.5202021.xyz
  - direct: yoyo-vps.5202021.xyz
...
```powershell
# 1. 启用隧道
Invoke-RestMethod -Uri "https://yoyoapi.5202021.xyz/api/admin/tunnel/toggle" `
...
- [ ] tunnel 返回: `https://tunnel-hls.yoyo-vps.5202021.xyz/hls/...`
- [ ] direct 返回: `https://yoyoapi.5202021.xyz/hls/...`
- [ ] **不包含** `/tunnel-proxy` 路径
...
3. **detectConnectionModeFromUrl()** (第505-557行)
   - 兜底方案：从HLS URL推断连接模式
   - `tunnel-hls.yoyo-vps.5202021.xyz` → `tunnel`
   - `yoyoapi.5202021.xyz/tunnel-proxy/` → `proxy` ❌ **废弃路径**
   - `yoyoapi.5202021.xyz` → `direct`
...
const detectConnectionModeFromUrl = (url) => {
  ...
  if (url.includes('tunnel-hls.yoyo-vps.5202021.xyz') || 
      url.includes('tunnel-api.yoyo-vps.5202021.xyz')) {
    frontend = 'tunnel'
  }
