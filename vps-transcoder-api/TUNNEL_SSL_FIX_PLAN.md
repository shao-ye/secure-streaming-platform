...
### **受影响端点**
- ❌ `https://tunnel-hls.yoyo-vps.5202021.xyz` - SSL握手失败
- ✅ `https://yoyoapi.5202021.xyz` - 正常工作（直连模式）
...
# SSL握手测试
curl tunnel-hls.yoyo-vps.5202021.xyz - SSL握手失败 ❌
```
...
#### **架构变更**
```
浏览器 --> yoyoapi.5202021.xyz/tunnel-proxy/* --> Cloudflare Worker --> Tunnel --> VPS
```
...
if (pathname.startsWith('/tunnel-proxy/')) {
  ...
  const tunnelUrl = `https://tunnel-hls.yoyo-vps.5202021.xyz${tunnelPath}`;
...
1. 访问 `https://yoyo.5202021.xyz/admin`
2. 进入"隧道优化"选项卡
3. 关闭隧道开关
4. 系统自动降级到直连模式
...
```bash
# 1. 测试SSL握手
curl -v https://tunnel-hls.yoyo-vps.5202021.xyz/

# 2. 测试HLS访问
curl -I https://tunnel-hls.yoyo-vps.5202021.xyz/hls/stream_test/playlist.m3u8
