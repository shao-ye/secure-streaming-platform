...
```
用户浏览器 → 前端展示层 → 业务逻辑层 → 转码服务层 → RTMP视频源
    ↓           ↓              ↓              ↓           ↓
 Vue播放器   Cloudflare   Cloudflare     VPS服务器   监控摄像头
            Pages         Workers       (FFmpeg)
```
...
```
RTMP源 → [路径1] → VPS转码 → [路径2] → Workers代理 → [路径3] → 前端播放器
   ↑                  ↑                    ↑                    ↑
监控摄像头          FFmpeg转HLS         业务逻辑处理          用户浏览器
```
...
| 路径段 | 起点 | 终点 | 作用 | 可优化点 |
|-------|------|------|------|---------|
| **路径1** | RTMP源 | VPS | 视频源到转码服务器 | 🔄 VPS代理 |
| **路径2** | VPS | Workers | HLS文件传输 | 🌐 Cloudflare Tunnel |
| **路径3** | Workers | 用户 | 最终分发 | ✅ Cloudflare CDN（默认） |

---

### 双维度网络优化设计 🆕

系统采用**两个独立的优化维度**，针对不同的网络路径段：

#### 维度1: 前端路径优化（Workers → VPS）- 路径2

**优化目标**: 改善Cloudflare Workers到VPS服务器的连接

| 模式 | 路径 | 优化方式 | 适用场景 |
|------|------|---------|---------|
| **隧道模式** | Workers → Cloudflare Tunnel → VPS | 通过Cloudflare骨干网传输 | 中国大陆用户 |
| **直连模式** | Workers → 直连 → VPS | 公网直接连接 | 海外用户或隧道关闭 |
...
#### 维度2: 后端路径优化（VPS → RTMP源）- 路径1

**优化目标**: 改善VPS到RTMP视频源的连接

| 模式 | 路径 | 优化方式 | 适用场景 |
|------|------|---------|---------|
| **代理模式** | VPS → V2Ray代理 → RTMP源 | 通过代理服务器中转 | RTMP源访问受限 |
| **直连模式** | VPS → 直连 → RTMP源 | 直接连接 | RTMP源可直接访问 |

**优化效果**：
- 突破访问限制：解决某些RTMP源的网络阻断
- 稳定性提升：代理服务器提供更稳定的连接
...
```
┌─────────────────────────────────────────────────────────────┐
│ 组合1: tunnel + proxy ⭐ 最优组合                             │
│ ════════════════════════════════════════════════════════════ │
│ RTMP源 → V2Ray代理 → VPS → Cloudflare Tunnel → Workers → 用户│
│         └──路径1优化──┘    └────路径2优化────┘                │
│ 效果: 前后端双优化，最佳性能和稳定性                            │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ 组合2: tunnel + direct ✅ 前端优化                            │
│ ════════════════════════════════════════════════════════════ │
│ RTMP源 → 直连 → VPS → Cloudflare Tunnel → Workers → 用户     │
│                      └────路径2优化────┘                      │
│ 效果: 前端加速，适合RTMP源可直接访问的场景                      │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ 组合3: direct + proxy 🟡 后端优化                            │
│ ════════════════════════════════════════════════════════════ │
│ RTMP源 → V2Ray代理 → VPS → 直连 → Workers → 用户             │
│         └──路径1优化──┘                                       │
│ 效果: 解决RTMP源访问问题，但Workers到VPS较慢                   │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ 组合4: direct + direct ⚠️ 无优化                            │
│ ════════════════════════════════════════════════════════════ │
│ RTMP源 → 直连 → VPS → 直连 → Workers → 用户                  │
│ 效果: 完全直连，性能依赖网络条件                               │
└─────────────────────────────────────────────────────────────┘
```
...
```
播放器左下角显示：
前端: [隧道优化] 或 [直连]       ← 显示路径2的状态
后端: [代理加速] 或 不显示        ← 显示路径1的状态（有代理时才显示）
```

**示例**：
```
场景1: tunnel + proxy
显示: 前端: [隧道优化]  后端: [代理加速]  延迟: 15ms

场景2: tunnel + direct  
显示: 前端: [隧道优化]  延迟: 18ms

场景3: direct + proxy
显示: 前端: [直连]  后端: [代理加速]  延迟: 120ms
...
1. **路径独立性**: 前端路径和后端路径互不影响，可以任意组合
2. **职责分离**: 
   - Workers只管理前端路径（tunnel/direct）
   - VPS只管理后端路径（proxy/direct）
3. **信息传递**: 后端状态通过HTTP响应头传递给前端展示
4. **URL简化**: 视频URL只反映前端路径（2种），不反映后端状态
...
### 第一层：前端展示层 (Cloudflare Pages)
- **域名**: `https://yoyo.5202021.xyz`
- **技术栈**: Vue.js 3 + Element Plus + hls.js
- **部署平台**: Cloudflare Pages
- **功能**: 用户界面、视频播放器、频道管理
- **在数据流中的位置**: 路径3终点（最终用户访问）

---

### 第二层：业务逻辑层 (Cloudflare Workers)
- **域名**: `https://yoyoapi.5202021.xyz`
- **技术栈**: Cloudflare Workers + KV存储
- **功能**: 
  - API服务、用户认证、会话管理
  - 业务逻辑处理、频道配置管理
  - **前端路径路由**（管理Workers→VPS连接）
  - HLS文件代理和智能故障转移
- **在数据流中的位置**: 路径2终点、路径3起点

**路由决策**：
- 隧道开关 + 地理位置 → 选择tunnel或direct
- 查询VPS代理状态 → 用于前端显示

---

### 第三层：转码服务层 (VPS)
- **直连域名**: `https://yoyo-vps.5202021.xyz`
- **隧道域名**: `https://tunnel-*.yoyo-vps.5202021.xyz` (前端路径优化)
- **服务器IP**: `142.171.75.220`
- **SSH端口**: `22` (标准SSH连接端口)
- **服务端口**: 
  - `3000` - Node.js API服务
  - `52535` - Nginx HLS文件服务
- **技术栈**: Node.js + Express + FFmpeg + Nginx + PM2 + V2Ray
- **功能**: 
  - RTMP到HLS转码（FFmpeg）
  - HLS文件服务（Nginx）
  - 进程管理（PM2）
  - **后端路径管理**（V2Ray代理到RTMP源）
- **在数据流中的位置**: 路径1终点、路径2起点

**路由选择**：
- V2Ray代理开关 → VPS通过proxy或direct访问RTMP源

#### VPS连接信息
- **SSH连接**: `ssh root@142.171.75.220`
- **SCP文件传输**: `scp file.js root@142.171.75.220:/path/to/destination/`
- **直连访问**: `https://yoyo-vps.5202021.xyz` (Nginx:52535)
- **隧道访问**: `https://tunnel-*.yoyo-vps.5202021.xyz` (Cloudflare Tunnel)
...
```
前端路径（Workers → VPS）- 路径2：
┌─────────────────────────────────────────────────┐
│ tunnel模式: Workers → tunnel-*.yoyo-vps.5202021.xyz │
│ direct模式: Workers → yoyo-vps.5202021.xyz          │
└─────────────────────────────────────────────────┘

后端路径（VPS → RTMP源）- 路径1：
┌─────────────────────────────────────────────────┐
│ proxy模式:  VPS → V2Ray代理 → RTMP源             │
│ direct模式: VPS → 直连 → RTMP源                   │
└─────────────────────────────────────────────────┘

前端展示（用户 → Workers）- 路径3：
┌─────────────────────────────────────────────────┐
│ 始终通过: 用户 → Cloudflare CDN → Workers        │
│ (此路径已由Cloudflare CDN自动优化)                │
└─────────────────────────────────────────────────┘
```
...
**架构说明**：系统采用**双维度独立优化**设计，前端路径和后端路径相互独立：

```mermaid
graph TD
    A[用户点击频道] --> B[前端请求: /api/simple-stream/start-watching]
    B --> C[Cloudflare Workers接收]
    C --> D[检查频道配置]
    D --> E[从KV读取频道配置]
    
    E --> F{前端路径选择}
    F -->|隧道开关ON + 中国用户| G[Workers通过隧道访问VPS]
    F -->|隧道开关OFF 或 海外用户| H[Workers直连VPS]
    
    G --> I[VPS启动转码进程]
    H --> I
    
    I --> J{后端路径状态}
    J -->|VPS代理已连接| K[VPS通过代理访问RTMP源]
    J -->|VPS无代理| L[VPS直连RTMP源]
    
    K --> M[返回基础HLS路径]
    L --> M
    
    M --> N{前端URL包装}
    N -->|隧道| O[tunnel-hls.yoyo-vps.5202021.xyz/hls/...]
    N -->|直连| P[yoyoapi.5202021.xyz/hls/...]
    
    O --> Q[前端获取HLS URL + 路由信息]
    P --> Q
...
- **tunnel**: 通过Cloudflare Tunnel访问VPS（中国用户优化）
- **direct**: Workers直连VPS（海外用户或隧道关闭）
- **控制方式**: KV开关 + 地理位置检测
...
**路径组合（2×2=4种）**：
```
1. tunnel + proxy ⭐ - 最优组合，前后端双优化
2. tunnel + direct ✅ - 前端优化，后端直连
3. direct + proxy 🟡 - 后端优化，前端直连
4. direct + direct ⚠️ - 无优化，完全直连
```

#### 智能路由核心特性
1. **双维度独立**: 前端路径和后端路径相互独立，可以任意组合
2. **统一接口**: 前端始终使用相同的API端点
3. **智能包装**: 只有两种URL（tunnel/direct），后端状态通过响应头传递
4. **故障转移**: 隧道失败时自动切换到直连
5. **完整信息**: 响应头包含前端和后端路径的完整状态
...
// 状态：connected（通过代理）或 direct（直连）
// 这个状态不影响路由决策，仅用于信息展示
```

**URL设计（只有两种）**：
```
前端路径: tunnel → https://tunnel-hls.yoyo-vps.5202021.xyz/hls/...
前端路径: direct → https://yoyoapi.5202021.xyz/hls/...

后端路径状态通过响应头传递，不体现在URL中
```
