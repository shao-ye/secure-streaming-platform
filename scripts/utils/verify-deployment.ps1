...
try {
    $apiStatus = Invoke-RestMethod -Uri "https://yoyoapi.your-domain.com/api/status" -Method GET -TimeoutSec 10
...
try {
    $frontendResponse = Invoke-WebRequest -Uri "https://yoyo.your-domain.com" -Method GET -TimeoutSec 10
...
try {
    $globalConfigResponse = Invoke-RestMethod -Uri "https://yoyoapi.your-domain.com/api/admin/proxy/global-config" -Method GET -Headers @{"Authorization"="Bearer invalid-token"} -TimeoutSec 10
} catch {
...
try {
    $testProxyResponse = Invoke-RestMethod -Uri "https://yoyoapi.your-domain.com/api/admin/proxy/test" -Method POST -Headers @{"Authorization"="Bearer invalid-token"} -TimeoutSec 10
} catch {
    if ($_.Exception.Response.StatusCode -eq 401 -or $_.ErrorDetails.Message -like "*AUTH_REQUIRED*") {
...
try {
    $historyResponse = Invoke-RestMethod -Uri "https://yoyoapi.your-domain.com/api/admin/proxy/test-history/test-proxy-id" -Method GET -Headers @{"Authorization"="Bearer invalid-token"} -TimeoutSec 10
} catch {
    if ($_.Exception.Response.StatusCode -eq 401 -or $_.ErrorDetails.Message -like "*AUTH_REQUIRED*") {
...
try {
    $vpsStatus = Invoke-RestMethod -Uri "https://yoyo-vps.your-domain.com/api/status" -Method GET -TimeoutSec 10
...
Write-Host "   前端: https://yoyo.your-domain.com" -ForegroundColor Blue
Write-Host "   API:  https://yoyoapi.your-domain.com" -ForegroundColor Blue
Write-Host "   VPS:  https://yoyo-vps.your-domain.com" -ForegroundColor Blue
Write-Host ""
